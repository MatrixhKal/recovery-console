...
int vt_init(DisplayDev *d) {
  ...
  /* We are active right now – grab DRM master if applicable. */
  if (d->is_drm)
    drm_set_master(d);
...
void vt_acquire(DisplayDev *d) {
  ...
  if (d->is_drm) {
    drm_set_master(d);
    drm_reprogram_crtc(d); /* re-attach our FB to CRTC after X owned it */
  }

  /* Full clear: wipes everything outside terminal margins and any pixels
   * left by the previous VT owner (X server, TWRP, etc.).             */
...
uint32_t blend_px(uint32_t bg, uint32_t fg, uint32_t a) {
  if (__builtin_expect(a == 0,   0)) return bg;
  if (__builtin_expect(a == 255, 0)) return fg;
  uint32_t inv = 255 - a;
...
static inline __attribute__((always_inline))
uint32_t *fb_px(int x, int y) {
#if ROTATION == 0
  return g_fb + (uint32_t)y * g_stride + (uint32_t)x;
#elif ROTATION == 1
  return g_fb + (uint32_t)x * g_stride + (uint32_t)(g_fb_w - 1 - y);
#elif ROTATION == 2
  return g_fb + (uint32_t)(g_fb_h - 1 - y) * g_stride + (uint32_t)(g_fb_w - 1 - x);
#elif ROTATION == 3
  return g_fb + (uint32_t)(g_fb_h - 1 - x) * g_stride + (uint32_t)y;
#else
  int rx = x, ry = y;
  if (ROTATION == 1) { rx = g_fb_w-1-y; ry = x; }
  else if (ROTATION == 2) { rx = g_fb_w-1-x; ry = g_fb_h-1-y; }
  else if (ROTATION == 3) { rx = y; ry = g_fb_h-1-x; }
...
static inline __attribute__((always_inline))
void hfill(int x, int y, int len, uint32_t col) {
#if ROTATION == 0
  uint32_t *p = g_fb + (uint32_t)y * g_stride + (uint32_t)x;
  for (int i = 0; i < len; i++) p[i] = col;
#else
  for (int i = 0; i < len; i++) *fb_px(x+i, y) = col;
#endif
}

static inline __attribute__((always_inline))
void vfill(int x, int y, int len, uint32_t col) {
#if ROTATION == 0
  uint32_t *p = g_fb + (uint32_t)y * g_stride + (uint32_t)x;
  for (int i = 0; i < len; i++) { *p = col; p += g_stride; }
#else
  for (int i = 0; i < len; i++) *fb_px(x, y+i) = col;
#endif
}

static inline __attribute__((always_inline))
uint32_t resolve_color(uint32_t c) {
...
void display_render(DisplayDev *d, Term *t) {
  ...
  for (int r = 0; r < t->rows; r++) {
    ...
    for (int c = 0; c < cols; c++) {
      ...
      if (__builtin_expect(cl->width == 0, 0)) continue;
      ...
      if (__builtin_expect((cl->attr & ATTR_DIM) && !(cl->attr & ATTR_REVERSE), 0))
        fg = blend_px(bg, fg, 128);

      /* Background fill */
      for (int y = 0; y < ch; y++) hfill(x0, y0+y, pw, bg);

      const uint32_t code = cl->code;
      if (__builtin_expect(code <= ' ' || code == 0xFEFF, 0)) goto cell_done;
      ...
      if (__builtin_expect(code >= 0x2800 && code <= 0x28FF, 0)) {
        ...
        for (int dr = 0; dr < 4; dr++)
          for (int dc = 0; dc < 2; dc++) {
            if (!((m >> braille_map[dc][dr]) & 1)) continue;
            int dx = x0 + dc*dw + dw/4;
            int dy = y0 + dr*dh + dh/4;
            for (int yy = 0; yy < doth; yy++) hfill(dx, dy+yy, dotw, fg);
          }
        goto cell_done;
      }
      ...
      if (__builtin_expect(code >= 0x2500 && code <= 0x257F, 0)) {
        ...
        if (code <= 0x2501)      flags = 3;
        else if (code <= 0x2503) flags = 12;
        else {
          static const uint8_t box_flags[] = {
            10,10,10,10, 9, 9, 9, 9,  6, 6, 6, 6,  5, 5, 5, 5, /* 250C-251B */
            14,14,14,14,14,14,14,14, 13,13,13,13,13,13,13,13,  /* 251C-252B */
            11,11,11,11,11,11,11,11,  7, 7, 7, 7, 7, 7, 7, 7,  /* 252C-253B */
            15,15,15,15,15,15,15,15, 15,15,15,15,15,15,15,15,  /* 253C-254B */
          };
          int idx = (int)(code - 0x250C);
          flags = (idx >= 0 && idx < (int)sizeof(box_flags))
                  ? box_flags[idx] : 3;
        }
        if (flags & 1) hfill(x0,    y0+my, mx+1,    fg);
        if (flags & 2) hfill(x0+mx, y0+my, pw-mx,   fg);
        if (flags & 4) vfill(x0+mx, y0,    my+1,    fg);
        if (flags & 8) vfill(x0+mx, y0+my, ch-my,   fg);
        goto cell_done;
      }
      ...
      if (__builtin_expect(code >= 0x2580 && code <= 0x2588, 0)) {
        int fh, ys;
        if (code == 0x2588)      { fh = ch;                    ys = 0; }
        else if (code == 0x2580) { fh = ch/2;                  ys = 0; }
        else                     { fh = ch*(int)(code-0x2580)/8; ys = ch-fh; }
        for (int y = ys; y < ys+fh; y++) hfill(x0, y0+y, pw, fg);
        goto cell_done;
      }
  ...
  if (t->cursor_visible && t->cy >= 0 && t->cy < t->rows &&
      t->cx >= 0 && t->cx < t->cols) {
    int x0 = t->cx * cw + MARGIN_LEFT;
    int y0 = t->cy * ch + MARGIN_TOP;
    uint32_t clr = palette[CURSOR_COLOR];
    for (int y = 0; y < ch; y++) hfill(x0, y0+y, cw, clr);
  }

  if (any) display_kick(d);
}

/*  display_blank  *
 *  blank=true  → black out screen (power save / user pressed power key).
 *  blank=false → hardware unblank; caller must trigger re-render to fill.
 */
void display_blank(DisplayDev *d, bool blank) {
  if (d->is_drm) {
    if (blank && g_fb && d->buf.size > 0) {
      memset(d->buf.map, 0, d->buf.size);
      display_kick(d);
    }
    /* unblank: caller re-renders with display_render(). */
  } else {
    fbdev_blank(d, blank);
  }
}
...
static void ensure_node(const char *path, int maj, int min) {
  ...
  if (stat(path, &st) == 0 && S_ISCHR(st.st_mode) &&
      major(st.st_rdev) == (unsigned)maj &&
      minor(st.st_rdev) == (unsigned)min)
    return;
  (void)mknod(path, S_IFCHR | 0666, makedev((unsigned)maj, (unsigned)min));
}

/*  display_init  *
 *  Tries DRM first (preferred – atomic KMS handoff with X/Wayland).
 *  Falls back to fbdev.  On success caches the framebuffer globals used
 *  by the hot-path render functions and builds the 256-colour palette.
 */
bool display_init(DisplayDev *d) {
  /* Optional: set backlight (best-effort, non-fatal). */
  FILE *bf = fopen(BACKLIGHT_PATH, "w");
  if (bf) { fprintf(bf, "%d\n", BACKLIGHT_VAL); fclose(bf); }
  ...
  /* Cache framebuffer globals for the hot render path. */
...
#if COLOR_BGR
    palette[i] = 0xFF000000u | (uint32_t)r | ((uint32_t)g2 << 8) | ((uint32_t)b << 16);
#else
    palette[i] = 0xFF000000u | (uint32_t)b | ((uint32_t)g2 << 8) | ((uint32_t)r << 16);
#endif
  }
